...
const Login = () => {
  ...
  const handlelogin = async () => {
    ...
    try{
        const res = await
    axios.post('https://fullstack-student-backend.onrender.com/api/auth/login', {email,password})
        console.log(res)
        navigate('/Home')
    }catch(err){
        console.error(err)
  ...
  return (
    <Box
      sx={{
        ...
        alignItems: "center",
        background: "linear-gradient(135deg, #1565c0, #75b6ebff)",
        ...
        >
         ...
         {error ? error : 'Welcome Back'}
          ...
          >
            ...
            >
             <Link to ='/sign' style={{color:'inherit'}}> Sign up</Link>
            </a>
          </Typography>
        </Paper>
      </motion.div>
